...
def read_json_data(file_path: str):
    ...
    [
        {"testName": "Valid login", "email": "test1@example.com", "password": "abc123", "expected": "success"},
        {"testName": "Invalid login", "email": "test2@example.com", "password": "xyz123", "expected": "failure"}
    ]
    """
    data = []
    try:
        ...
        for record in json_data:
            data.append(tuple(str(value) for value in record.values()))  # convert all values to string
...
def read_csv_data(file_path: str):
    ...
    CSV file should contain headers: testName, email, password, expected
    """
    data = []
    try:
        ...
        for row in reader:
            data.append(tuple(str(value) for value in row.values()))  # convert all values to string
    except Exception as e:
...
def read_excel_data(file_path: str, sheet_name: str = None):
    ...
    Assumes the first row contains headers (testName, email, password, expected).
    All values are converted to strings to avoid type errors in Playwright.
    """
    data = []
    try:
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            data.append(tuple(str(cell) for cell in row))  # convert all values to string
    except Exception as e:
